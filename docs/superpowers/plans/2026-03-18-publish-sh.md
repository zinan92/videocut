# publish.sh Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a `publish.sh` script that guides users through publishing videocut output to 4 platforms (抖音, 小红书, 公众号, X), with optional `bird` CLI auto-publish for X.

**Architecture:** Pure bash script using `pbcopy` (clipboard) + `open` (browser) for semi-auto publishing on 3 platforms, with `bird` CLI detection for optional X auto-publish. Reads content from the output directory, walks user through each platform sequentially, and updates `manifest.json` status after each.

**Tech Stack:** Bash, pbcopy, open (macOS built-in), jq-free JSON via node -e, optional bird CLI
...
|--------|------|----------------|
| Create | `publish.sh` | Main publishing script — 4 platform publish flow |
| Modify | `pipeline.sh:88-107` | Add `--publish` flag to parameter parsing |
| Modify | `pipeline.sh:631-633` | Call publish.sh after Phase 6 summary if `--publish` |
| Modify | `README.md` | Document publish.sh usage + optional bird CLI |

---

## Task 1: Create `publish.sh` — core structure + helpers

**Files:**
- Create: `publish.sh`

- [ ] **Step 1: Create publish.sh with header, argument parsing, and helper functions**
...
```bash
git add publish.sh
git commit -m "feat: create publish.sh with core structure and helpers"
```

---

## Task 2: Add 抖音 publishing

**Files:**
- Modify: `publish.sh` (append)

- [ ] **Step 1: Add publish_douyin function**

Append to `publish.sh` before the final `echo ""`:
...
```bash
git add publish.sh
git commit -m "feat: add 抖音 semi-auto publishing to publish.sh"
```

---

## Task 3: Add 小红书 publishing

**Files:**
- Modify: `publish.sh` (append)

- [ ] **Step 1: Add publish_xhs function**

Append after `publish_douyin`:
...
bash -n publish.sh
git add publish.sh
git commit -m "feat: add 小红书 semi-auto publishing to publish.sh"
```

---

## Task 4: Add 公众号 publishing

**Files:**
- Modify: `publish.sh` (append)

- [ ] **Step 1: Add publish_wechat function**
...
bash -n publish.sh
git add publish.sh
git commit -m "feat: add 公众号 semi-auto publishing to publish.sh"
```

---

## Task 5: Add X/Twitter publishing (with bird auto-detect)

**Files:**
- Modify: `publish.sh` (append)

- [ ] **Step 1: Add publish_x function with bird detection**
...
bash -n publish.sh
git add publish.sh
git commit -m "feat: add X/Twitter publishing with bird CLI auto-detect"
```

---

## Task 6: Add main execution flow + summary

**Files:**
- Modify: `publish.sh` (append main flow at end)

- [ ] **Step 1: Add main execution and summary**

Append at the end of `publish.sh`:
...
- [ ] **Step 2: Verify syntax, test with missing dir**

```bash
bash -n publish.sh
bash publish.sh 2>&1 | head -3  # should show error about missing dir
```
...
```bash
git add publish.sh
git commit -m "feat: add main execution flow and summary to publish.sh"
```

---

## Task 7: Add `--publish` flag to `pipeline.sh`

**Files:**
- Modify: `pipeline.sh:88-107` (parameter parsing)
- Modify: `pipeline.sh:82-85` (script paths)
- Modify: `pipeline.sh:631-633` (end of file, after summary)

- [ ] **Step 1: Add PUBLISH_SH path**

After line 85 (`CARDS_SH="$SCRIPT_DIR/generate-cards.sh"`), add:

```bash
PUBLISH_SH="$SCRIPT_DIR/publish.sh"
```

- [ ] **Step 2: Add --publish flag to parameter parsing**

In the `case` block (~line 92-107), add a new case before the `-*` catch-all:
...
Also add the variable initialization after `OUTPUT_DIR_OVERRIDE=""` (~line 90):

```bash
DO_PUBLISH=false
```

- [ ] **Step 3: Call publish.sh at the end of pipeline**

After the final `echo ""` at line 633 (end of file), append:
...
- [ ] **Step 4: Update the usage comment at the top of pipeline.sh**

Line 3-6 header comment, add `--publish` to the options list:

```bash
#   --publish            Phase 6 后启动发布流程
```
...
```bash
git add pipeline.sh
git commit -m "feat: add --publish flag to pipeline.sh for Phase 7 publishing"
```

---

## Task 8: Add douyin platform to manifest.json generation

**Files:**
- Modify: `pipeline.sh` (manifest generation node script, ~line 525-580)

Currently manifest.json has `jike`, `xhs`, `wechat`, `x_post`, `x_thread`, `youtube`, `bilibili`, `podcast` but no `douyin`. Need to add it.

- [ ] **Step 1: Add douyin to the manifest platforms object**

In the manifest generation node script, after the `jike` entry and before `xhs`, add:
...
bash -n pipeline.sh
git add pipeline.sh
git commit -m "feat: add douyin platform to manifest.json"
```
...
- [ ] **Step 1: Add publish.sh documentation**

Add a new section after the "功能一览" table. Find the line with `| Phase 6 |` and add after the table:
...
- [ ] **Step 2: Add publish.sh to the subcommands section**

In the "Subcommands" YAML block, add:
...
Add Phase 7 row to the feature table:
...
```bash
git add README.md
git commit -m "docs: add publish.sh documentation to README"
```
...
bash -n publish.sh && echo "publish.sh OK"
bash -n pipeline.sh && echo "pipeline.sh OK"
```

- [ ] **Step 2: Test publish.sh with missing directory**

```bash
bash publish.sh 2>&1 | head -3
```
Expected: error about missing output directory

- [ ] **Step 3: Test publish.sh --platform filter**

```bash
bash publish.sh /nonexistent --platform x 2>&1 | head -3
```
Expected: error about directory not existing

- [ ] **Step 4: Test pipeline.sh still works without --publish**

```bash
bash pipeline.sh 2>&1 | head -3
```
Expected: error about missing video path (same as before)
